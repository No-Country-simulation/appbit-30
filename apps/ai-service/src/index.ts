...
// Valores de nota inferidos por emoji (según documento de salud)
...
app.post('/wellbeing/analyze', async (c) => {
  try {
    ...
    if (!validation.success) {
      return c.json({ error: 'Datos inválidos', details: validation.error.format() }, 400);
    }
    ...
    const tendencia_baja = historial_semanal.length > 1 && historial_semanal.every(
      (val, i) => i === 0 || val <= historial_semanal[i - 1]!
    );

    // 4. Reglas de derivación y alerta
    let derivar_cvv = nota_semanal < 4.0 || (tendencia_baja && nota_semanal < 5.0);
